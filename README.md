...
The goal is simple. With the help of native speakers we want to make Biblians usable and natural in as many languages as possible.
...
If you are a native or very fluent speaker of any of these, your help is welcome.
...
Each file is a flat JSON of `key` and `"translated text"` pairs.  
The English file `locales/en.json` is the main reference for meaning and context, so please always compare your language with that file while translating.
...
2. Check the English file for context  
   Open `locales/en.json` to understand what each key means, then open the file for your language in the same folder.

3. Edit the JSON file for your language  
   - Only change the values, not the keys  
   - Keep placeholders like `{name}` and `{count}` exactly as they are  
   - Aim for clear, natural language instead of word for word translation  

4. Check the JSON is valid  
   - Make sure there are no missing commas or quotes  
   - Use any JSON aware editor or online validator if needed  
...
- Keep the tone warm, respectful and friendly, fitting a Bible chat and study app  
...
- For texts related to personal reflections or confessions, use gentle and non judgmental wording  
...
Thank you for helping more people read, chat about and study the Bible in their own language with Biblians.
